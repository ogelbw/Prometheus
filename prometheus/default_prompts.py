...
from prometheus.tools.definitions import InstructionResponse, LLMToolParameter, instruction
...
def _makeToolPromptDefault(
    ...
    return [
        ...
        {"role": "system", "content": f"The script must have a global function called 'Run' so it can be called by the system. 'Run' must call the function you make and it must return a string (even if it is empty). This is because stdout is ignored by the system."},
        {"role": "system", "content": f"The python script must be runnable on linux and windows systems, it should detect which system it is on and act accordingly."},
...
def _makePlanPromptDefault(final_goal: str, plan: List[instruction], tools: List[dict]):
    return [
        ...
        {"role": "system", "content": f"The system has access to these tools: [{', '.join(tools.keys())}]"},
...
def _updatePlanPromptDefault(final_goal: str, plan: List[instruction], previous_action_response: InstructionResponse, tools: List[dict]):
    return [
        {"role": "system", "content": f"You are a AI planner part of a larger system. This system can make it's own tools in python and is already logged into the user's machine."},
        {"role": "system", "content": f"The user gave the system this goal: <{final_goal}>."},
        {"role": "system", "content": f"This is the current plan to achieve the goal:\n{[f"{i}. {x.action}, Reason: {x.reason}" for i, x in enumerate(plan)]}"},
...
def _takeActionStepPromptDefault(step: instruction, how: str, executionHistory: List[InstructionResponse], goal: str):
    return [
        {"role": "system", "content": f"You are an AI responsible for taking actions in a system."},
        {"role": "system", "content": f"Here is the previous execution history of the system: {". ".join([x.response for x in executionHistory])}"},
